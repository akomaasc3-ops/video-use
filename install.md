...
- HyperFrames, Remotion, Motion Canvas, and Manim are optional animation engines. Don't install or prefer one globally during setup; pick the engine per animation slot in `SKILL.md`. HyperFrames can run through `npx --yes hyperframes ...` in the slot directory. Remotion can be scaffolded with `npx create-video@latest` (or `npm install remotion @remotion/cli react react-dom` inside the slot) then rendered with `npx remotion render <CompositionId> out.mp4`. Motion Canvas can be scaffolded with `npm init @motion-canvas@latest` inside the slot, then `npm install`; it renders an image sequence (or uses its bundled `@motion-canvas/ffmpeg` package) which the slot's render step muxes into video.
- Sandboxed/offline environments: `remotion render` downloads its own Chrome Headless Shell from `remotion.media` on first use. If that host isn't on the network allowlist (403 / "Host not in allowlist"), point Remotion at a browser already on disk instead of widening egress: `npx remotion render <CompositionId> out.mp4 --browser-executable=/path/to/chrome-or-headless-shell` (or set `browserExecutable` in `remotion.config.ts`). A Playwright Chromium install (`$PLAYWRIGHT_BROWSERS_PATH/chromium-*/chrome-linux/chrome` or the `chromium_headless_shell-*` sibling) works fine as the executable.
